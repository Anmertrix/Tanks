...
public class Level {
	...
	public Level(TextureAtlas atlas) {
		...
		tiles.put(TileType.ISE, new Tile(atlas.cut(36 * TILE_SCALE, 4 * TILE_SCALE, TILE_SCALE, TILE_SCALE),
				TILE_IN_GAME_SCALE, TileType.ISE ));
		...
		glassCords = new ArrayList<Point>();
		for (int i=0; i < tileMap.length; i++) {
			for (int j=0; j < tileMap[i].length; j++) {
				Tile tile = tiles.get(TileType.fromNumeric(tileMap[i][j]));
				if (tile.type() == TileType.GRASS)
					glassCords.add(new Point(j * SCALED_TILE_SIZE, i * SCALED_TILE_SIZE));
			}
		}
	}
